...
class DecidableRequestAppealTab extends StatefulWidget {
  DecidableRequestAppealTab({Key? key}) : super(key: key);

  @override
  _DecidableRequestAppealTabState createState() => _DecidableRequestAppealTabState();
}

class _DecidableRequestAppealTabState extends State<DecidableRequestAppealTab> {
  late DateTime fromDate;
  late DateTime toDate;
  bool showFilter = false;

  int? attendanceAppealRequestsLength;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: Refreshable(
              key: context.read<KeyProvider>().decidableAppealKey,
              child: CustomPagedListView<MyRequestsResponseDTO>(
                initPageFuture: (pageKey) async {
                  var decidableResult = await ApiRepo().getDecidableRequests(
                    approver: true,
                    requestKind: ["ATTENDANCE_APPEAL_REQUEST"],
                  ...
                  attendanceAppealRequestsLength =
                      (attendanceAppealRequestsLength ?? 0) + (decidableResult?.result?.records?.length ?? 0);
                  return decidableResult.result!.toPagedList();
                },
                itemBuilder: (context, item, index) {
                  return DecidableRequestCard(
                    myRequestsResponseDTO: item,
                    refreshableKey: context.read<KeyProvider>().decidableAppealKey,
                    totalLength: attendanceAppealRequestsLength,
                    index: index,
                  );
                },
              ),
