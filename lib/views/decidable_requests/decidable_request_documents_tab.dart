...
class DecidableDocumentsTab extends StatefulWidget {
  DecidableDocumentsTab({Key? key}) : super(key: key);

  @override
  _DecidableDocumentsTabState createState() => _DecidableDocumentsTabState();
}

class _DecidableDocumentsTabState extends State<DecidableDocumentsTab> {
  late DateTime fromDate;
  late DateTime toDate;
  bool showFilter = false;

  int? documentsDecidableLength;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: Refreshable(
              key: context.read<KeyProvider>().decidableDocumentKey,
              child: CustomPagedListView<MyRequestsResponseDTO>(
                initPageFuture: (pageKey) async {
                  ...
                  documentsDecidableLength =
                      (documentsDecidableLength ?? 0) + (decidableResult?.result?.records?.length ?? 0);
                  return decidableResult.result!.toPagedList();
                },
                itemBuilder: (context, item, index) {
                  return DecidableRequestCard(
                    myRequestsResponseDTO: item,
                    refreshableKey: context.read<KeyProvider>().decidableDocumentKey,
                    totalLength: documentsDecidableLength,
                    index: index,
                  );
                },
              ),
