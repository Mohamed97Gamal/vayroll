...
class DecidableRequestExpenseTab extends StatefulWidget {
  @override
  _DecidableRequestExpenseTabState createState() => _DecidableRequestExpenseTabState();
}

class _DecidableRequestExpenseTabState extends State<DecidableRequestExpenseTab> {
  late DateTime fromDate;
  late DateTime toDate;
  bool showFilter = false;

  int? expensesDecidableLength;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: Refreshable(
              key: context.read<KeyProvider>().decidableExpenseKey,
              child: CustomPagedListView<MyRequestsResponseDTO>(
                initPageFuture: (pageKey) async {
                  ...
                  expensesDecidableLength =
                      (expensesDecidableLength ?? 0) + (decidableResult?.result?.records?.length ?? 0);
                  return decidableResult.result!.toPagedList();
                },
                itemBuilder: (context, item, index) {
                  return DecidableRequestCard(
                    myRequestsResponseDTO: item,
                    refreshableKey: context.read<KeyProvider>().decidableExpenseKey,
                    totalLength: expensesDecidableLength,
                    index: index,
                  );
                },
              ),
