...
class DecidableRequestLeaveTab extends StatefulWidget {
  @override
  _DecidableRequestLeaveTabState createState() => _DecidableRequestLeaveTabState();
}

class _DecidableRequestLeaveTabState extends State<DecidableRequestLeaveTab> {
  late DateTime fromDate;
  late DateTime toDate;
  bool showFilter = false;

  int? leaveDecidavleLength;
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(
            child: Refreshable(
              key: context.read<KeyProvider>().decidableLeaveKey,
              child: CustomPagedListView<MyRequestsResponseDTO>(
                initPageFuture: (pageKey) async {
                  ...
                  leaveDecidavleLength = (leaveDecidavleLength ?? 0) + (decidableResult?.result?.records?.length ?? 0);
                  return decidableResult.result!.toPagedList();
                },
                itemBuilder: (context, item, index) {
                  return DecidableRequestCard(
                    myRequestsResponseDTO: item,
                    refreshableKey: context.read<KeyProvider>().decidableLeaveKey,
                    totalLength: leaveDecidavleLength,
                    index: index,
                  );
                },
              ),
