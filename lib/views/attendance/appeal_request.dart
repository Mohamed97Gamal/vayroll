...
class AttendanceAppealRequestPage extends StatefulWidget {
  final CalendarAttendance? dayAttendance;

  AttendanceAppealRequestPage({Key? key, this.dayAttendance}) : super(key: key);

  @override
  _AttendanceAppealRequestPageState createState() => _AttendanceAppealRequestPageState();
}

class _AttendanceAppealRequestPageState extends State<AttendanceAppealRequestPage> {
  ...
  void initState() {
    super.initState();
    checkInTime = TimeOfDay.fromDateTime(widget.dayAttendance!.checkInTime ?? DateTime.now());
    checkOutTime = TimeOfDay.fromDateTime(
        widget.dayAttendance!.checkOutTime ?? widget.dayAttendance!.checkInTime!.add(Duration(minutes: 1)));
  }

  bool _isSameData(CalendarAttendance originalData, DateTime newCheckIn, DateTime newCheckOut) {
    return (isSameTime(originalData.checkInTime, newCheckIn) && isSameTime(originalData.checkOutTime, newCheckOut));
  }
  ...
  void _onSubmit() async {
    ...
    var empId = context.read<EmployeeProvider>().employee!.id;
    var checkIn = DateTime.utc(widget.dayAttendance!.date!.year, widget.dayAttendance!.date!.month,
        widget.dayAttendance!.date!.day, checkInTime!.hour, checkInTime!.minute);
    var checkOut = DateTime.utc(widget.dayAttendance!.date!.year, widget.dayAttendance!.date!.month,
        widget.dayAttendance!.date!.day, checkOutTime!.hour, checkOutTime!.minute);

    if (_isSameData(widget.dayAttendance!, checkIn, checkOut)) {
      await showCustomModalBottomSheet(
        context: context,
        desc: context.appStrings!.weCannotProceedWithYourRequestAsYouDidNotChangeCheckInOutTime,
    ...
    final response = (await showFutureProgressDialog<BaseResponse<String>>(
      context: context,
      initFuture: () =>
          ApiRepo().attendanceAppealRequest(empId, widget.dayAttendance!.date!, checkIn, checkOut, note, attachment),
    ))!;
        ...
        child: ListView(
          ...
          children: [
            InnerTextFormField(
              ...
              initialValue: dateFormat.format(widget.dayAttendance!.date!),
            ),
